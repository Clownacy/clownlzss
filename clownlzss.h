...
\
	...
	{\
		/* String list stuff */\
		size_t* const next = malloc(((MAX_MATCH_DISTANCE) + 0x100) * sizeof(size_t));\
		size_t* const prev = malloc((MAX_MATCH_DISTANCE) * sizeof(size_t));\
		size_t* const bytes = malloc((MAX_MATCH_DISTANCE) * sizeof(size_t));\
\
