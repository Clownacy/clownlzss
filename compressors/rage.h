...
namespace ClownLZSS
{
	namespace Internal
	{
		namespace Rage
		{
			...
			bool Compress(const unsigned char* const data, const std::size_t data_size, T &&output)
			{
				...
				if (!ClownLZSS::FindOptimalMatches(0xFFFFFFFF/*dictionary-matches can be infinite*/, 0x1FFF, FindExtraMatches, 0xFFFFFFF/*dummy*/, GetMatchCost, data, 1, data_size, &matches, &total_matches, nullptr))
					return false;
	...
	template<typename T>
	bool ModuledRageCompress(const unsigned char* const data, const std::size_t data_size, T &&output, const std::size_t module_size, const std::size_t module_alignment)
	{
		using namespace Internal;

		return ModuledCompressionWrapper(data, data_size, CompressorOutput(output), Rage::Compress, module_size, module_alignment);
	}
}
