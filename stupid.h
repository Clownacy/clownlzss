void CLOWNLZSS_NAME(CLOWNLZSS_TYPE *data, size_t data_size, size_t max_match_length, size_t max_match_distance, size_t literal_cost, unsigned int (*cost_callback)(size_t distance, size_t length), void (*literal_callback)(CLOWNLZSS_TYPE value), void (*match_callback)(size_t longest_match_index, size_t longest_match_length))
{
	NodeMeta *node_meta_array = malloc((data_size + 1) * sizeof(NodeMeta));	// +1 for the end-node
